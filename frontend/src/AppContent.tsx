...
export default function App({ token, username, onLogout }: AppProps) {
  ...
  async function loadLogs(page: number = 1) {
    try {
      const res = await authFetch(`/api/logs?page=${page}`);
      if (!res.ok) {
  ...
  async function hardDeleteMatch(id: number) {
    if (!confirm("Permanently delete this match? This cannot be undone.")) return;
    console.log("hardDeleteMatch called with ID:", id);
    try {
      const url = `/api/matches/${id}?hard=true`;
      console.log("Sending hard DELETE request to:", url);
      const res = await authFetch(url, { method: "DELETE" });
      if (!res.ok) {
        const text = await res.text();
        console.error("Hard delete match failed:", res.status, text);
        alert(`Failed to delete match: ${text}`);
        return;
      }
      console.log("Hard delete successful, reloading matches...");
    } catch (err) {
      console.error("Hard delete match error:", err);
  ...
  async function removeUrl(id: number) {
    ...
    await authFetch(`/api/urls/${id}`, { method: "DELETE" });
    await loadUrls();
  }
  ...
  async function triggerWorker() {
    try {
      const res = await authFetch("/api/trigger-worker", { method: "POST" });
      const data = await res.json();
  ...
  async function testSMS() {
    try {
      const res = await authFetch("/api/test-sms", { method: "POST" });
      if (!res.ok) {
        const text = await res.text();
    ...
    <div className="container">
      <div className="header">
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <button onClick={onLogout} className="run-button" style={{ padding: '8px 16px', fontSize: '14px' }}>
            Logout
          </button>
        </div>
      </div>
